...
- `dev/build/transform.py` — builds the app: takes the Hanbit source, deletes the
  ...
  `hapkido-part1..5.js` before BOOT (later function declarations override the
  originals — that's the override mechanism), swaps the mascot SVG
...
- `dev/build/hapkido-part1..5.js` — the hapkido layer: catalogue/tracks/planner
  (1), exercises/session rendering (2), path/belt/detail views (3), practice/
...
- `dev/hapkido-companion/data/curriculum.js` — ALL content. Belts (data-driven:
  colors/order/gup/units), domains (with `track: knowledge|technique` driving the
  two-lane path), items (term/concept/technique). Ladder rungs auto-skip when an
  item lacks the data (no stepSequence → no sequencing rung).
...
```bash
python3 dev/build/transform.py        # rebuilds both index.html files
node dev/build/test-matrix.js         # 59 checks; needs playwright + chromium
```

The matrix must pass 59/59 (grow it with every feature — count goes up, never
down). It covers: boot, FSRS values against hand-computed expectations, wrong-answer
requeue, step sequencing, self-graded speaking, belt gating, cumulative counts,
two-track filtered sessions, practice logging + restricted-technique gating,
instructor PIN/verify/unverify/belt-advance, export/import round-trip, malformed
import, orphan-card retirement, missing/broken media states, themes, 320px,
reduced motion, keyboard, reset, corrupted-save recovery. Deploy = commit + push
(Pages redeploys root `index.html` automatically). Report honestly what was and
wasn't tested; headless can't test TTS or real video playback.
