...
function renderExercise(step) {
  ...
  if (ex.type === 'mc') {
    ...
    // Once checked, mark the student's OWN answer: each step is either in the
    // right place or shows the place it belongs. Ordering five steps is a lot
    // of reading, and diffing it against the correct list underneath is more
    // reading again — this puts the whole result where they already are.
    // Colour is never the only signal: the tick and the target number say the
    // same thing, and each button carries an aria-label for screen readers.
    const graded = !!sess.reveal && ex.stepMode && full;
    answerBlock = `<div class="build-target steps" id="buildtarget">${chosen.map((t, i) => {
      const ok = t.i === i;
      const mark = !graded ? ''
        : ok ? '<span class="stepmark ok" aria-hidden="true">✓</span>'
             : `<span class="stepmark off" aria-hidden="true">→&nbsp;${t.i + 1}</span>`;
      const label = !graded ? ''
        : ok ? ` aria-label="Position ${i + 1}, correct: ${esc(t.w)}"`
             : ` aria-label="Position ${i + 1}, misplaced — belongs at position ${t.i + 1}: ${esc(t.w)}"`;
      return `<button class="tile step${graded ? (ok ? ' ok' : ' off') : ''}" data-unpick="${i}"${
        sess.reveal ? ' disabled' : ''}${label}><span class="stepnum">${i + 1}.</span><span>${esc(t.w)}</span>${mark}</button>`;
    }).join('')
      || '<span class="faint" style="font-size:13px;align-self:center">tap the steps in order, first to last</span>'}</div>
      ${sess.reveal ? '' : `<div class="build-pool steps">${ex.tiles.map(t =>
      `<button class="tile step ${chosen.some(c => c.i === t.i) ? 'used' : ''}" data-pickt="${t.i}">${esc(t.w)}</button>`).join('')}</div>`}
      ${graded ? `<div class="step-tally">${chosen.filter((t, i) => t.i === i).length} of ${chosen.length} in the right place</div>` : ''}
...
function renderFeedback(ex) {
  ...
  let answerLine = '';
  if (ex.stepMode) {
    // Dim the positions the student already had right so the eye goes straight
    // to the ones that moved. Only meaningful when they actually attempted a
    // full order — after "Show me" there is nothing to compare against.
    const chosen = sess.build || [];
    const graded = ex.orderAnswer.length > 0 && chosen.length === ex.orderAnswer.length && r.result !== 'shown';
    answerLine = `<ol class="fb-steps${graded ? ' marked' : ''}">${ex.orderAnswer.map((s, i) => {
      if (!graded) return `<li>${esc(s)}</li>`;
...
/* How fast counts as "easy" on a sequencing card.

   The engine grades multiple choice 4 (easy) under 3.2s — right for a short
   prompt and four short options. Sequencing inherited a flat 14s, which is a
   different job entirely: read every step, find each one in a shuffled pool,
   and tap them in order. A five-step technique is ~40 words read at least
   twice, so a fluent student still lands past 14s and gets 3 — meaning the
   longest exercise in the app could effectively never earn the longer
   interval, and kept coming back to be read again.

   Scale the allowance with the actual reading load instead: orientation, plus
   per-tile scan-and-tap, plus reading time at roughly 230 wpm. Being fast
   still has to mean fast — this only stops charging the student for how much
   text the item happens to contain. */
function sequenceFastMs(ex) {
  const tiles = ex.tiles || [];
  const words = tiles.reduce((n, t) => n + String(t.w).trim().split(/\s+/).filter(Boolean).length, 0);
  return 3500 + tiles.length * 900 + words * 260;
}
...
function submitAnswer() {
  ...
  const grade = result === 'right' ? (elapsed < sequenceFastMs(ex) ? 4 : 3) : 1;
